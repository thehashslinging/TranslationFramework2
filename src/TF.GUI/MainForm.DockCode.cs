    ...
    partial class MainForm
    {
        ...
        private void ExplorerOnRestoreItem(object selectedNode)
        {
            if (selectedNode is TranslationFileContainer container)
            {
                var result =
                    MessageBox.Show(
                        "Ini akan memulihkan file asli dalam penampung ini. Operasi ini tidak dapat dibatalkan.\nApakah Anda ingin melanjutkan?",
                        "Pulihkan file", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            if (selectedNode is TranslationFile file)
            {
                var result =
                    MessageBox.Show(
                        "Ini akan mengembalikan file asli. Operasi ini tidak dapat dibatalkan.\nApakah Anda ingin melanjutkan?",
                        "Pulihkan file", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private bool CloseAllDocuments()
        {
            if (_currentFile != null)
            {
                ...
                {
                    var result = MessageBox.Show("Ada perubahan yang tertunda pada file.\nApakah Anda ingin menyimpannya?",
                        "Simpan perubahan", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
